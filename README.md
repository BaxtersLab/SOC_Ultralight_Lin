...
Tile the agent windows side by side with no overlap, and keep the SOC control
panel on its own edge so it never covers a region SOC has to read. Each agent
needs its input field, send button and scroll control visible without scrolling
the window itself.
...
┌───────────────────────────┬───────────────────────────┬─────────┐
│  Agent 1                  │  Agent 2                  │   SOC   │
│  chat client              │  chat client              │ control │
│                           │                           │  panel  │
│  · input field            │  · input field            │         │
│  · send button            │  · send button            │ Set Win │
│  · scroll ↑ / ↓           │  · scroll ↑ / ↓           │ per     │
│                           │                           │ agent   │
└───────────────────────────┴───────────────────────────┴─────────┘
```
...
```
┌────────────────────┬────────────────────┬────────────────┬─────────┐
│  Agent 1           │  Agent 2           │  Agent 3       │   SOC   │
│  chat client       │  chat client       │  chat client   │ control │
│                    │                    │                │  panel  │
│  · input field     │  · input field     │  · input field │         │
│  · send button     │  · send button     │  · send button │ Set Win │
│  · scroll ↑ / ↓    │  · scroll ↑ / ↓    │  · scroll ↑/↓  │ per     │
│                    │                    │                │ agent   │
└────────────────────┴────────────────────┴────────────────┴─────────┘
```

Narrower columns mean smaller OCR regions, so 3-agent benefits from a wide or
high-resolution display. Use **Set Win** per agent to calibrate each one after
arranging; the saved coordinates follow the window, so a later move is fine.
